...
- `_opLocks` (`src/disk.ts`) is a per-URI counter incremented **synchronously** in the `asset:file:update` listener and decremented in `finally` after `_update` resolves. `OTDocument._text` mutates synchronously when a remote op arrives, but `_locks.add()` only runs inside `_update`'s async mutex callback — leaving a sync-to-microtask gap where `contentChanges` offsets (relative to the pre-op buffer) misalign with `file.doc.text` (post-op). The keystroke handler must short-circuit on **either** `_locks.has(uri)` **or** `_opLocks.has(uri)`; the counter also stays > 0 across consecutive in-flight `_update`s so the gap doesn't reopen between them.
...
- **Server is authoritative on reconciliation.** On `_create`, `_subscribed`, and closed-file remote updates, divergent disk content is overwritten by the server snapshot — never push local disk upstream as part of open/subscribe/reopen. Local edits reach the server only via `onDidChangeTextDocument` (keystrokes on open files), explorer create/delete/rename, and Cmd+S save. External edits to closed files do not sync in realtime — the watcher surfaces a one-shot warning pointing at Pull/Push and drops the edit. Accept that narrow loss rather than risk collab data loss from stale local files clobbering server state.
...
- `_diskHash` must be deleted whenever the file is deleted, renamed, or unlinked. Stale entries cause false "divergent" merges on re-create.
- Watcher `create` events for paths already present in `ProjectManager.files` must never fall through to `projectManager.create` — a temp+rename over an existing asset would duplicate the asset server-side. The atomic-write guard returns early instead.
...
- **Server-Authoritative Reconciliation**: On open/subscribe/reopen, server content overwrites divergent disk/buffer. Local edits reach the server only via keystrokes on open files; create/delete/rename still flow through the watcher, but external edits to closed files are dropped with a one-shot warning.
...
- **Closed-file external edits**: not synced in realtime. The watcher shows a one-shot warning (`_warnExternalEdit`) pointing at Pull/Push and leaves the edit on disk.
- **Disk sync**: Debounced writes pre-set the echo hash; the watcher skips matching or superseded events. `_diskHash` is cleared on delete/rename/unlink so re-creates don't see false divergence.
